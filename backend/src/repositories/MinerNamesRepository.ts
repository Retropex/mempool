...
class MinerNamesRepository {
  ...
  /**
   * Blocks per miner over a ranking interval, biggest first. Shaped like
   * PoolsRepository.$getPoolsInfo, and read from `blocks_miners` alone.
   *
   * @asyncSafe
   */
  public async $getMinersInfo(interval: string | null = null): Promise<MinerInfo[]> {
    ...
    let query = `SELECT
      ...
      WHERE blocks_miners.stale = 0`; // the join drops pool-built blocks, whose miner_id is null

    if (interval) {
      query += ` AND blocks_miners.blockTimestamp BETWEEN DATE_SUB(NOW(), INTERVAL ${interval}) AND NOW()`;
    }

    // biggest miner first, name breaking ties so the order is stable across requests
    query += ` GROUP BY blocks_miners.pool_id, blocks_miners.miner_id
      ORDER BY blockCount DESC, name ASC`;

    try {
      const [rows]: any[] = await DB.query(query);
      return rows;
    } catch (e) {
      logger.err(`Cannot generate miners stats. Reason: ` + (e instanceof Error ? e.message : e));
      throw e;
    }
  }
